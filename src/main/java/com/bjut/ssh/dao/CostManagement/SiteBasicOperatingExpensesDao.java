...
/**
 * @Title: SiteBasicOperatingExpensesDao
 * @Description: TODO
 * @Author: zf
...
@Repository
@Transactional
public class SiteBasicOperatingExpensesDao {
    @Autowired
    ...
    public Msg getValData(String selectSalesMethod, String selectTimeField, String selectDepartmentValue){
        ...
        if(value_List.size()>0){
            tx.commit();
            close(session);
            return  Msg.success().add("SBOE_value_List",value_List);
        }
        else{
            tx.commit();
            close(session);
...
//                    System.out.println(val_List);
        ...
        if (value_List.size() > 0) {
            tx.commit();
            close(session);
            return Msg.success().add("SBOE_datagridValue_List", value_List);
        } else {
            tx.commit();
            close(session);
            return Msg.fail().add("errorInfo", "编号重复，请重新输入");
    ...
    private List<List<String>> getDepartment(String sqlFiled_Department){
        ...
        System.out.println(sqlFiled_Department);
...
//                    System.out.println(k + " : " + m.get(k)); // 测试
            ...
            System.out.println(sqlFiled_Department);//测试
            try {
