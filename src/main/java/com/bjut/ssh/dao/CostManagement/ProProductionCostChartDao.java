...
/**
 * @Title: ProProductionCostChartDao
 * @Description: TODO
 * @Author: zf
...
@Repository
@Transactional
public class ProProductionCostChartDao {
    @Autowired
    ...
    public Msg getProduct(){
        ...
        if(list.size()>0){
            tx.commit();
            close(session);
            return  Msg.success().add("product_List",list);
        }
        else{
            tx.commit();
            close(session);
...
//        System.out.println(value_List);
        ...
        if(value_List.size()>0){
            tx.commit();
            close(session);
            return  Msg.success().add("PPCC_value_List",value_List);
        }
        else{
            tx.commit();
            close(session);
...
//        System.out.println(value_List);
        ...
        if(value_List.size()>0){
            tx.commit();
            close(session);
            return  Msg.success().add("PPCC_datagridValue_List",value_List);
        }
        else{
            tx.commit();
            close(session);
    ...
    private Double getSumProduction(String sqlFiled_Production, String departmentID){
        ...
        System.out.println("Sum" + departmentID);
