...
/**
 * @Title: SiteProductionCostStructureDao
 * @Description: TODO
 * @Author: zf
...
@Repository
@Transactional
public class SiteProductionCostStructureDao {
    @Autowired
    ...
    public Msg getShowNo(String selectSiteValue){
        ...
        items = getItemNames(selectSiteValue);
    ...
    public Msg getSiteSelect(String selectDepartmentValue){
        System.out.println("getSiteSelect");
        Session session = null;
        Transaction tx = null;
        session = getSession();
        tx = session.beginTransaction();
        ...
        if(selectDepartmentValue.equals("All")){
            ...
            index = selectDepartmentValue.substring(0,4);
            try {
                query = session.createQuery("select spName,spNo from Lshszd where catNo = :a and spLevel = :b and substring(spNo,1,4)=(" + index + ")");
                query.setString("a", "01");
                query.setInteger("b", 2);
                list = query.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
            }catch(Exception e){
...
//        System.out.println(list);
        if(list.size()>0) {
            tx.commit();
            close(session);
            return Msg.success().add("siteSelect",list);
        }
        else {
            close(session);
            return Msg.fail().add("errorInfo","编号重复，请重新输入");
...
//            System.out.println(sqlField_Summation);//测试
        ...
        for(String item : itemsID_List){
            lsVal1_List.add(getItemsMoney(sqlField_Summation, selectSiteValue, item));
        }
        ...
        return Msg.success().add("SPCS_value_List",value_List);//命名的前面是这个功能名的缩写
    }
...
//        System.out.println(value_List);
        ...
        if(value_List.size()>0){
            tx.commit();
            close(session);
            return  Msg.success().add("SPCS_datagridValue_List",value_List);
        }
        else{
            tx.commit();
            close(session);
