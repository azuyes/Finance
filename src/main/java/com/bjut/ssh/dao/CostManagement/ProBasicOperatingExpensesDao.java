...
/**
 * @Title: ProBasicOperatingExpensesDao
 * @Description: TODO
 * @Author: zf
...
@Repository
@Transactional
public class ProBasicOperatingExpensesDao {
    @Autowired
    ...
    public Msg getValData(String selectSalesMethod, String selectTimeField){
        ...
        if(value_List.size()>0){
            tx.commit();
            close(session);
            return  Msg.success().add("PBOE_value_List",value_List);
        }
        else{
            tx.commit();
            close(session);
    ...
    public Msg getDatagridData(String selectTimeField) {
        ...
        System.out.println(selectTimeField);
        ...
        if (value_List.size() > 0) {
            tx.commit();
            close(session);
            return Msg.success().add("PBOE_datagridValue_List", value_List);
        } else {
            tx.commit();
            close(session);
            return Msg.fail().add("errorInfo", "编号重复，请重新输入");
