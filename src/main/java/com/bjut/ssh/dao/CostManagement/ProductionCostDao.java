...
@Repository
@Transactional
public class ProductionCostDao {
    ...
    public Msg getEchartsTheme(){
        ...
        query1.setString("a","000010");
        String graphTheme1 = (String) query1.uniqueResult();
        themeList.add(graphTheme1);

        tx.commit();
        close(session);
        return Msg.success().add("ProductionCost_getEchartsTheme",themeList);
    }
    ...
    public Msg getDepartmentSelect(){
        ...
        if(list.size()>0) {
            tx.commit();
            close(session);
            return Msg.success().add("ProductionCost_departmentSelect",list);
        }
        else {
            tx.commit();
            close(session);
    ...
    public Msg getProductSelect(){
        ...
        if(list.size()>0) {
            tx.commit();
            close(session);
            return Msg.success().add("ProductionCost_productSelect",list);
        }
        else {
            tx.commit();
            close(session);
    ...
    public Msg getItemNam(){
        ...
        //普通科目
        String itemNo1list = getGraphData("1", "000009");
        if (itemNo1list != null) {
        ...
        //自定义科目
        String itemNo2list = getGraphData("2", "000009");
        if (itemNo2list != null) {
        ...
        return Msg.success().add("ProductionCost_itemNam", itemNameList);
    }
    ...
    public Msg getValData(String selectCompany, String selectProduct, String selectTime){
        ...
        return Msg.success().add("ProductionCost_value_List",value_List);//命名的前面是这个功能名的缩写
    }
    ...
    public Msg getDatagridData(String selectCompany, String selectProduct, String selectTime){
        ...
        return Msg.success().add("ProductionCost_datagridValue_List",value_List);//命名的前面是这个功能名的缩写
    }
    ...
    public List<String> getItemNo(String itemType) {
        ...
        if (itemType.equals("1")) {
            String itemNolist1 = getGraphData("1", "000009");
        ...
        if (itemType.equals("2")) {
            String itemNolist2 = getGraphData("2", "000009");
