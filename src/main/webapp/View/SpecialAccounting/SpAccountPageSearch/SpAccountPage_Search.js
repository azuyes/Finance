...
$(function() {
    ...
    $('#spNo1_Input').textbox('textbox').bind('dblclick', function() {
        ...
        if(no != ""){
            ...
            $.messager.alert('提示！', '请输入核算类别编号！', 'info');
        }
    ...
    $('#main_table').datagrid({
        ...
        columns: [
            [{
                ...
                hidden: true,
                formatter: function (value, row, index) {
                    if(row.lspzk1['pzk1AutoId'] < 0){
                        return row.qtyJ;
                    }
                    else if (row.lspzk1['bkpDirection'] == 'J')
                ...
                formatter: function (value, row, index) {
                    ...
                    else if (row.lspzk1['bkpDirection'] == 'D')
                        return row.lspzk1['qty'];
                }
            }, {
                field: 'creditMoney',
...
function searchClick(){
    ...
    // }
    //
    // //第三种查询情况：输入科目编号二级为通配符，输入核算分类编号，核算编号
    // if(itemNo.lastIndexOf("....") != -1 && spNo1 == "..." ){
    //     search2();
    //     $('#balanceSearch_Win').window('close');
    //     document.getElementById("mainDisplay").style.visibility = "visible";
    //     return;
...
function querySpAccountPage1(){
    ...
    params.spNo1 = spNo1;
    params.spCatNo2 = null;
    params.spNo2 = null;

    $.ajax({
        type:'post',
        url: getRealPath() + '/SpAccountPageSearch/querySpAccountPage1',
        data: params,
        dataType:"json",
...
function queryMultiCol(from, to, itemNo, catNo1){
