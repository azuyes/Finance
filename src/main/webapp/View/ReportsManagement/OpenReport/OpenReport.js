...
$(document).ready(function() {
    ...
    var BBYear=BBDate.substring(0,4);//报表年份
    var BBMonth=BBDate.substring(5,7);//报表月份
    var BBYear_temp=BBDate.substring(0,4);//报表年份（Lsconf中查询的报表年份）

    //打开报表窗口 年份加载
    $('#yearTime').numberspinner({
        value:Number(BBDate.substring(0,4)),
        min:2000,
        max:Number(BBDate.substring(0,4)),
        precision:0,
        onChange:function () {
            BBYear = Number($('#yearTime').numberbox('getValue'))+"";
            var opt = $('#openBox').datagrid('options');
            opt.url = getRealPath() + '/ReportBase/findRptByYearAndMonth/' + BBYear + "/" + BBMonth;
            $('#openBox').datagrid('reload');
        }
    });

    //打开报表窗口 月加载
    $('#monthTime').numberspinner({
        value:Number(BBMonth),
        min:1,
        max:12,
        precision:0,
        onChange:function () {
            BBMonth = Number($('#monthTime').numberbox('getValue'))+"";
            if(Number(BBMonth)<10){
                BBMonth="0"+BBMonth;
            }
            var opt = $('#openBox').datagrid('options');
            opt.url = getRealPath() + '/ReportBase/findRptByYearAndMonth/' + BBYear + "/" + BBMonth;
            //$('#openBox').datagrid({url: getRealPath()+'/ReportBase/findRptByYearAndMonth/'+BBYear+"/"+BBMonth});
            $('#openBox').datagrid('reload');
        }
    });
    ...
    //打开报表窗口 数据加载
    $('#openBox').datagrid({
        url: getRealPath()+'/ReportBase/findRptByYearAndMonth/'+BBYear+"/"+BBMonth,
        fitColumns:true,
        striped: true,
        rownumbers: true,
        loadMsg: '正在加载中，请稍后',
    ...
    //打开报表窗口 确定按钮 点击事件
    $('#openReportBt').click(function () {
        var row = $('#openBox').datagrid('getSelected');
        if(row){
            BBNo = row.rptNo;
            BBDate = BBYear+"-"+BBMonth;
            $.ajax({
                type: 'POST',
                url: getRealPath() + '/ReportBase/openRptByNoAndDate',
                ...
                success: function (result) {
                    if(result.code==100){
                        ...
                        BBNo=lcbzd.rptNo;//获取报表编号
                        BBName=lcbzd.rptName;//获取报表标题
                        rows = Number(lcbzd.totalRules);//获取总行数
                        titleRows = Number(lcbzd.titleRules);//获取标题行数
                        headRows = Number(lcbzd.headRules);//获取表头行数
                        contentRows = Number(lcbzd.bodyRules);//获取表体行数
                        endRows = rows - titleRows - headRows - contentRows;//获取表尾行数
                        cols=Number(lcbzd.rptCol);//获取总列数
                        //alert(BBNo+"===="+BBName+"===="+BBDate+"===="+rows+"===="+cols);
                        ...
                        for (var i=0;i<lcdyzdList.length;i++) {
                            rowTemp = Number(lcdyzdList[i].rowInfoNo);
                            colTemp = Number(lcdyzdList[i].colInfoNo);
                            cell_id[rowTemp][colTemp] = lcdyzdList[i].dyzdAutoId;
                            cell_data[rowTemp][colTemp] = lcdyzdList[i].cellContent;
                            cell_formula[rowTemp][colTemp] = lcdyzdList[i].formula;
                            formula_range[rowTemp][colTemp] = lcdyzdList[i].formulaRange;
                            cell_height[rowTemp]=lcdyzdList[i].cellHeight;
                            cell_width[colTemp]=lcdyzdList[i].cellWidth;
                            font_style[rowTemp][colTemp]=lcdyzdList[i].fontStyle;
                            cell_className[rowTemp][colTemp]=lcdyzdList[i].cellClassName;
                            cell_type[rowTemp][colTemp]=lcdyzdList[i].cellType;
                            if((lcdyzdList[i].endRow!=null&&lcdyzdList[i].endRow!="") || (lcdyzdList[i].endCol!=null&&lcdyzdList[i].endCol!="")){
                                var mergeCell = [rowTemp,colTemp,Number(lcdyzdList[i].endRow),Number(lcdyzdList[i].endCol)];
                                cell_merge.push(mergeCell);
                            }
                        }
                        rptInit();
                        $('#rptDateNow').html("当前会计日期：" + BBYear + "-" + BBMonth);//函数窗口--显示会计日期
                        $('#dlg').dialog('close');
