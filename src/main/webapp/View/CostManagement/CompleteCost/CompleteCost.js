...
$(function () {
    ...
    $('#Button_ToReport').click(function () {
        ...
        var FileName;
        if (selectTimeValue === "AnnualCumulative") {
            FileName = year + "年完全成本信息及分析";
        } else {
            month = now.getMonth() + 1;
            FileName = year + "年" + month + "月完全成本信息及分析";
        }
...
function getEchartsTheme() {
    $.ajax({
        type: "POST",
        url: getRealPath() + '/CompleteCost/getEchartsTheme/',
        contentType: 'application/json',
        async: true,
        success: function (result) {
            if (result.code == 100) {
                var data1 = result.extend.CompleteCost_getEchartsTheme;
...
function departmentSelect() {
    $.ajax({
        type: "POST",
        url: getRealPath() + '/CompleteCost/getDepartmentSelect.action/',
        contentType: 'application/json',
        success: function (result) {
            if (result.code == 100) {
                var data1 = result.extend.CompleteCost_departmentSelect;
...
function productSelect() {
    $.ajax({
        type: "POST",
        url: getRealPath() + '/CompleteCost/getProductSelect.action/',
        contentType: 'application/json',
        success: function (result) {
            if (result.code == 100) {
                var data1 = result.extend.CompleteCost_productSelect;
                console.log(data1);
...
function Get_itemNam() {
    $.ajax({
        type: "POST",
        url: getRealPath() + '/CompleteCost/getItemNam/',
        contentType: 'application/json',
        success: function (result) {
            if (result.code == 100) {
                itemNam = result.extend.CompleteCost_itemNam;
...
function echartsDataSelect() {
    ...
    $.ajax({
        type: "POST",
        //async: false,
        url: getRealPath() + '/CompleteCost/getValData/' + selectCompanyValue + '/' + selectProductValue + '/' + timeValue,
        contentType: 'application/json',
        success: function (result) {
            if (result.code == 100) {
                chartsData = result.extend.CompleteCost_value_List;
...
function datagridDataSelect() {
    ...
    $.ajax({
        type: "POST",
        //async: false,
        url: getRealPath() + '/CompleteCost/getDatagridData/' + selectCompanyValue + '/' + selectProductValue + '/' + timeValue,
        contentType: 'application/json',
        success: function (result) {
            if (result.code == 100) {
                datagridData = result.extend.CompleteCost_datagridValue_List;
...
function myCharts2() {
    ...
    option2 = {
        ...
        title: {
            text: year + '年度' + month + '方气完全成本信息及分析',
            subtext: '',
            x: 'center',
            textStyle: {
                //fontSize: 18,
