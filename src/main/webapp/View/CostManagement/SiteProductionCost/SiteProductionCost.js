...
$(function () {
    ...
    $('#Button_ToReport').click(function () {
        ...
        if(selectTime === "AnnualCumulative"){
            ...
            month = now.getMonth() + 1;
            FileName = year + "年" + month + "月" + name + "部门生产成本构成情况";
        }
        myDataGrids(FileName);
...
function geteChartsData() {
    ...
    $.ajax({
        type: "POST",
        url: getRealPath()+'/SiteProductionCost/getValData/'+ selectSalesMethodValue + '/' + timeField + '/' + selectDepatmentValue,
        contentType: 'application/json',
        success: function(result) {
            if (result.code == 100) {
                itemsVal = result.extend.SPC_value_List;
                nam = itemsVal[0];
...
function getItemsName(){
    $.ajax({
        type: "POST",
        url: getRealPath()+'/SiteProductionCost/getItemNam/',
        contentType: 'application/json',
        success: function(result) {
            if (result.code == 100) {
...
function datagridDataSelect() {
    $.ajax({
        type: "POST",
        //async: false,
        url: getRealPath()+'/SiteProductionCost/getDatagridData/' + selectDepatmentValue + '/' + selectTimeField,
        contentType: 'application/json',
        success: function(result) {
            if (result.code == 100) {
                datagridData  = result.extend.SPC_datagridValue_List;
