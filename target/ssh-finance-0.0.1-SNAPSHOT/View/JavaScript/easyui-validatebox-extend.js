$.extend($.fn.validatebox.defaults.rules, {
    decimal: {// 验证一位小数 
        ...
        message: '请输入最多保留1位的小数'
    },
    decimalOfTree: {// 验证3位小数 
        validator: function (value) {
            return /^-?\d+\.?\d{0,3}$/.test(value);
        },
        message: '请输入最多保留3位的小数'
    },
    idcard: {// 验证身份证 
        validator: function (value) {
    ...
    numberOfDigits: {// 验证数字位数
        validator: function (value, param) {  
            if (value.length < param[0] || value.length > param[1]) {  
                $.fn.validatebox.defaults.rules.numberOfDigits.message = '长度必须为' + param[0] + '位';
                return false;  
            } else {  
                if (!/^[0-9]*$/.test(value)) {
                    $.fn.validatebox.defaults.rules.numberOfDigits.message = '只能输入数字';
                    return false;  
                } else {  
                    return true;  
                }  
